...
/// GZ#1508 — a customer ("Kate") who holds TWO active memberships of the same
/// type (same membership_id 421) — one freshly topped up with hours, one
/// already at 0 hours — saw only ONE of them in the app, and it was the
/// 0-hour pass. The new pass was invisible even though the backend returned
/// BOTH rows in `activeMembership`.
///
...
void main() {
  ...
  UserActiveMembership kate() => UserActiveMembership(
        membershipModel: [],
        ...
        activeMembership: [
          ActiveMemberships(
            id: 15645,
            membershipId: 421,
            membershipName: 'Padel Rush Pass',
  ...
  group('GZ#1508 multiple active memberships of the same type', () {
    test('activeMembershipsFor returns EVERY active row for a membership_id',
        () {
      final visible = kate().activeMembershipsFor(421);
      expect(visible.length, 2,
          reason: 'both active Padel Rush Pass rows must be visible');
    });
    ...
    test('returns empty when the customer has no active row for that id', () {
      expect(kate().activeMembershipsFor(999), isEmpty);
    });
  });
}
